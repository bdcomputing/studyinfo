        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white  overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-primary ">
                    <div class="py-5">
                        <p class="text-lg md:text-xl">Update City</p>
                    </div>
                    <form id="form" action="{{ route('admin.cities.update',$city) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="">
                                ...
                                <input type="text" name="name" id="title" value="{{ old('name',$city->name) }}" required
                            ...
                            <div class="">
                                ...
                                <select name="destination_id" class="rounded-md border-gray-400 w-full">
                                    <option class="p-3">Select Country</option>
                                    @forelse($countries as $country)
                                    <option {{ old('destination_id',$city->destination_id) === $country->id ?'selected' : '' }} value="{{$country->id}}">{{$country->name}}</option>
                                    @empty
